  ...
  */
  ...
  var Searcher = function(prefs) {
    ...
    this.search = function(cb) {
      // TODO: Implement this.
      $.get('http://0.0.0.0:5000/search?' + params(this.prefs), function(data) {
        console.log(data);
      }, function(err) {
        console.log(err);
      })
  ...
  var params = function(data) {
    var countParam = 'count=' + data.count;
    var radiusParam = 'radius=' + data.radius;
    var latParam = 'lat=' + data.position.lat;
    var lngParam = 'lng=' + data.position.lng;
    var tagsParam = 'tags=' + data.tags.join();
    var getParams = [countParam, radiusParam, latParam, lngParam, tagsParam].join('&');
